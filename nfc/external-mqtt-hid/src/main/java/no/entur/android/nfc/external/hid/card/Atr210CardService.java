...
public class Atr210CardService {
    ...
    public Atr210CardService(Context context, SynchronizedRequestResponseMessages<String> adpuRequestResponseMessages, MqttServiceClient mqttClient, long transcieveTimeout, INFcTagBinder infcTagBinder, TagProxyStore tagProxyStore, boolean logApdus) {
    ...
    public void createTag() {
        ...
        LOGGER.info("Got tag type " + tagType + " from " + ByteArrayHexStringConverter.toHexString(cardContext.getAtr()));
        ...
        if(tagType == TagType.ISO_DEP || tagType == TagType.DESFIRE_EV1 || tagType == TagType.ISO_14443_TYPE_A) {
            ...
            if(tagType == TagType.DESFIRE_EV1) {
                wrapper = new Atr210DesfireWrapper(commands, logApdus);
            } else {
                wrapper = new Atr210IsoDepWrapper(commands, logApdus);
            }
