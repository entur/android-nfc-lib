 ...
 * <p>
 * Tags that enumerate the {@link IsoDepImpl} technology in {@link TagImpl#getTechList} will also enumerate {@link NfcAImpl} or {@link NfcB} (since IsoDep
...
public class IsoDepImpl extends IsoDep {
	 ...
	 * <p>
	 * The historical bytes can be used to help identify a tag. They are present only on {@link IsoDepImpl} tags that are based on {@link NfcAImpl} RF
	 * technology. If this tag is not {@link NfcAImpl} then null is returned.
	 * <p>
	 ...
	 * Does not cause any RF activity and does not block.
	 * <p>
	 * The higher layer response bytes can be used to help identify a tag. They are present only on {@link IsoDepImpl} tags that are based on {@link NfcB} RF
	 * technology. If this tag is not {@link NfcB} then null is returned.
	 * <p>
	 ...
	 *
	 * <p>
	 * Applications must only send the INF payload, and not the start of frame and end of frame indicators. Applications do not need to fragment the payload, it
	 * will be automatically fragmented and defragmented by {@link #transceive} if it exceeds FSD/FSC limits.
	 *
	 * <p>
	...
	public byte[] transceive(byte[] data) throws IOException {
		byte[] transceive = delegate.transceive(data, true);
	 ...
	 *
	 * <p>
	 * Some NFC adapters, like the one used in the Nexus S and the Galaxy Nexus do not support extended length APDUs. They are expected to be well-supported in
	 * the future though. Use this method to check for extended length APDU support.
	 *
