 ...
 *
 * <p>
 * MIFARE Ultralight compatible tags have 4 byte pages {@link #PAGE_SIZE}. The primary operations on an Ultralight tag are {@link #readPages} and
 * {@link #writePage}.
 *
 * <p>
 * The original MIFARE Ultralight consists of a 64 byte EEPROM. The first 4 pages are for the OTP area, manufacturer data, and locking bits. They are readable
 * and some bits are writable. The final 12 pages are the user read/write area. For more information see the NXP data sheet MF0ICU1.
 *
 * <p>
 * The MIFARE Ultralight C consists of a 192 byte EEPROM. The first 4 pages are for OTP, manufacturer data, and locking bits. The next 36 pages are the user
 * read/write area. The next 4 pages are additional locking bits, counters and authentication configuration and are readable. The final 4 pages are for the
 * authentication key and are not readable. For more information see the NXP data sheet MF0ICU2.
 *
 * <p>
 * Implementation of this class on a Android NFC device is optional. If it is not implemented, then {@link MifareUltralightImpl} will never be enumerated in
 * {@link TagImpl#getTechList}. If it is enumerated, then all {@link MifareUltralightImpl} I/O operations will be supported. In either case, {@link NfcAImpl}
 * will also be enumerated on the tag, because all MIFARE Ultralight tags are also {@link NfcAImpl} tags.
 *
 * <p class="note">
...
public final class MifareUltralightImpl extends MifareUltralight {
	 ...
	 * <p>
	 * Depending on how the tag has been formatted, it can be impossible to accurately classify between original MIFARE Ultralight and Ultralight C. So treat
	 ...
	 * <p>
	 * If a read spans past the last readable block, then the tag will return pages that have been wrapped back to the first blocks. MIFARE Ultralight tags have
	 * readable blocks 0x00 through 0x0F. So a read to block offset 0x0E would return blocks 0x0E, 0x0F, 0x00, 0x01. MIFARE Ultralight C tags have readable
	 * blocks 0x00 through 0x2B. So a read to block 0x2A would return blocks 0x2A, 0x2B, 0x00, 0x01.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This is equivalent to connecting to this tag via {@link NfcAImpl} and calling {@link NfcAImpl#transceive}. Note that all MIFARE Classic tags are based on
	 * {@link NfcAImpl} technology.
	 *
	 * <p>
	...
	public void setTimeout(int timeout) {
		try {
			int err = delegate.getTag().getTagService().setTimeout(TagTechnology.MIFARE_ULTRALIGHT, timeout);
