...
public class Acr1255UReader extends AcrReader {
	...
	public Acr1255UReader(String name, String id, IAcr1255UReaderControl readerControl) {
        super(id, name);
	...
	public static final Parcelable.Creator<Acr1255UReader> CREATOR = new Parcelable.Creator<Acr1255UReader>() {
		@Override
		public Acr1255UReader createFromParcel(Parcel in) {
			...
			return new Acr1255UReader(name, id, iin);
	...
	public boolean setAutomaticPICCPollingSpeed(AcrCommunicationSpeed tx, AcrCommunicationSpeed rx) {
		...
		try {
			...
			if (tx == AcrCommunicationSpeed.RATE_106_KBPS) {
				...
				throw new IllegalArgumentException("Unexpected rate " + tx);
			...
			if (rx == AcrCommunicationSpeed.RATE_106_KBPS) {
				...
				throw new IllegalArgumentException("Unexpected rate " + rx);
	...
	private AcrCommunicationSpeed parse(byte b) {
		if (b == RATE_106K) {
			...
			throw new IllegalArgumentException("Unexpected communication speed " + Integer.toHexString(0xFF & b));
	}
	...
	public boolean setSleepModeOption(int seconds) {
		...
		/*
		 * 00h = 60 seconds 01h = 90 seconds 02h = 120 seconds 03h = 180 seconds 04h = No Sleep
		 */
		byte[] response;
		try {
