...
import no.entur.android.nfc.external.hwb.intent.HwbNfcTagCallback;
...
public class HwbCardService implements SynchronizedRequestMessageListener<UUID> {
    ...
    protected final HwbMifareUltralightTagServiceSupport ultralightTagServiceSupport;
    ...
    protected MqttServiceClient mqttServiceClient;
    ...
    public HwbCardService(Context context, SynchronizedRequestResponseMessages<UUID> adpuRequestResponseMessages, MqttServiceClient mqttServiceClient, long transcieveTimeout, INFcTagBinder infcTagBinder, TagProxyStore tagProxyStore) {
        ...
        this.ultralightTagServiceSupport = new HwbMifareUltralightTagServiceSupport(context,  infcTagBinder, tagProxyStore, new HwbTransceiveResultExceptionMapper());
    ...
    public void createTag(String travelCardNumber, String token, List<CardContent> cardContent) {
        ...
        this.currentCard = isoDepTagServiceSupport.card(0, wrapper, cardContext.getUid(), cardContext.getHistoricalBytes(), (intent) -> {
            ...
            intent.putExtra(HwbNfcTagCallback.EXTRA_HWB_DEVICE_ID, cardContext.getDeviceId());
            if (token != null) {
                intent.putExtra(HwbNfcTagCallback.EXTRA_HWB_TOKEN, token);
            }
            if (travelCardNumber != null) {
                intent.putExtra(HwbNfcTagCallback.EXTRA_HWB_TRAVEL_CARD_NUMBER, travelCardNumber);
            }
            if (cardContent != null) {
                intent.putExtra(HwbNfcTagCallback.EXTRA_HWB_CARD_CONTENT, cardContent.toArray(new CardContent[cardContent.size()]));
            }

            intent.putExtra(HwbNfcTagCallback.EXTRA_TRACE_ID, cardContext.getTraceId().toString());
